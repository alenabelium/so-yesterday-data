---
title: "I Just Did a Full Day of Analyst Work in 10 Minutes. The $120K Job Description Just Changed Forever."
video_id: U1oHRqUkI1E
date: 2026-02-13
url: https://www.youtube.com/watch?v=U1oHRqUkI1E
tags:
  - industry-news
  - ai-strategy
  - ai-tools
  - career
transcript: ../transcripts/2026-02-13-i-just-did-a-full-day-of-analyst-work-in-10-minutes-the-120k.md
---

# I Just Did a Full Day of Analyst Work in 10 Minutes. The $120K Job Description Just Changed Forever.

## Executive Summary

Anthropic's Claude integration with Excel and PowerPoint represents a paradigm shift in knowledge work, enabling a full operating model and board deck to be built in 30 minutes -- work that would take a Goldman Sachs analyst a full day. Claude in Excel reads existing data, writes formulas, builds pivot tables, and debugs complex spreadsheets, while Claude in PowerPoint reads your slide masters, fonts, and templates to produce brand-consistent presentations. The real story is the emergence of a "context layer" between applications and data, where AI's accumulated understanding of your business compounds with every model upgrade -- and it improves automatically without users doing anything when new models ship.

## Key Points

- Goldman Sachs has deployed Claude in production for accounting and compliance, AIG reports 5x faster document reviews with accuracy improving from 75% to 90%, and Norway's sovereign wealth fund saved an estimated 213,000 hours -- proving this is not a demo but a production tool.
- The combination of Excel and PowerPoint under one intelligence eliminates the "translation layer" where knowledge workers spend most production hours converting analysis into different formats for different audiences, collapsing the data-to-decision pipeline.
- As AI makes artifact production nearly free, the scarce and valuable skill becomes judgment -- knowing which questions to ask, which analysis matters, and which output serves a real purpose -- while the execution premium that defined knowledge work for 30+ years evaporates.
